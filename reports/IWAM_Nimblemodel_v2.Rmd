...
pred.line <- data.frame(expand.grid(logWA = 1:16-mean_logWA, type = 1:2))
...
stock_recruit_srep_biasCor <- nimbleCode({
  ...
  for( i in 1:npred.line){
    ...
    b_line[i] <- -log_alpha_line[i]/E_pred_line[i]
    predSgen_line[i] <- -1/b_line[i]*RLambertsW(-b_line[i]*predSmsy_line[i]/exp(log_alpha_line[i]))
  }
  ...
  for( i in 1:npred){
    ...
    bpred[i] <- -log_alpha[i]/E_pred[i]
    predSgen[i] <- -1/bpred[i]*RLambertsW(-bpred[i]*predSmsy[i]/exp(log_alpha[i]))  
  }
