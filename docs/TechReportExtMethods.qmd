...
## Alternative Assumptions of Productivity

### Assumptions of productivity overview

This method was adapted, initially by Holt et al. (2023), into the assumption that infers an $\alpha$ based on the watershed-area model's mean predictions of $S_{MSY}$ and $S_{REP}$. The result is simulated estimates of $\alpha$, of which the median (invariant to monotonic-transforms) is taken and used to re-calculate $S_{MSY}$ and $S_{REP}$. These are optimized to find $S_{GEN}$ using the new estimates of $\alpha$, $\beta$, and $S_{MSY}$. Because of its dependence on the ratio of $S_{MSY}$ and $S_{REP}$, and ultimately shared information of $\alpha$ and $\beta$, this value changes depending on the base model parameterization used, even if apparent differences are small. This point is particularly relevant when comparing the models of Parken et al. (2006) and Liermann et al. (2010), which utilize different Ricker parameterizations.  

### The Scheurell et al. (2016) Solution

There is an explicit solution for $\alpha$, adapted from Scheurell et al. (2016), that is being tested for numerical accuracy that may supplement this method in the future.
...
### Prior and Posterior Checks, and Model Evaluation
To duplicate the same checks done by Liermann et al. (2010), we tested Geweke (1992) and Heidelberger and Welch (1981)  convergence statistics, both tests found evidence for poor chain convergence and stability. It is difficult to conclude with absolute certainty that all chains have converged, however, Rhat and visual inspection of trace plots  suggests that convergence has occurred, and that there are no glaring issues. Despite improvements to model encoding and more efficient samplers, the model remains complex and appears to have instances of difficult mixing. Even by increasing the number of MCMC iterations, as recommended by Geweke (1992) and Heidelberger and Welch (1981), from 5,000 to 500,000 minimal improvements in convergence statistics are made. We added Gelman-Rubin statistics (Gelman and Rubin 1992) of potential scale reduction factors to our suite of checks, and did not find improvement with additional iterations.
