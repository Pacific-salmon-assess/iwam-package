---
title: "Technical Report Extended Results Section"
format: 
    html:
        toc: true
        toc-title: Contents
...
<!--
-   trace plot per parameter (HTML): A visual convergence check to determine whether the model has had divergences, difficulties mixing, drift, or other MCMC sampling errors. Show per model parameter.
-   Autocorrelation plots per parameter (HTML): Shows per parameter, the autocorrelation coefficient against lag (in iterations of sampling). A measure of how much each sample in chain depends on the other. To prove model convergence.
-   pairs() (HTML): The pairwise comparison of sampled parameters from the MCMC draws. Used to visually determine how related parameters are and whether the chain has explored the posterior space. To prove model convergence.
-   Geweke, Heidel statistics (in Table for HTML) – matching the convergence diagnostic statistics used by Liermann et al.. Can also include Gelman diagnostic statistics. Described in Methods section. Used to diagnose stability/convergence/accuracy of MCMC chains. Not used a lot in current workflows, but can supply code in HTML file to cover the bases.
-   Prior predictive of logRS (HTML): To illustrate the importance of prior checks. A representation of how the model simulates observations without seeing the data. In our case, shows how uninformed the priors are.
-   Posterior distribution of k (logSREP_RE) against WA (See Liermann): To determine if there are patterns in the relationship between capacity and watershed size.
-   Residuals of posterior predictive against year and spawners (Panel for each synoptic stock) - looking for trends in the residuals with WA (See Liermann): To assess model fit and determine if there are patterns in the relationship. Liermann also used posterior predictive p-values to assess fit.
-->
...
```{r pval resid logsrep_re, echo = FALSE, results = 'asis', warning = FALSE, message = FALSE}
# Posterior P-value for residauls of logSREP_re by watershed area and latitude
```

### Posterior P-Values of logRS Residuals

```{r pval hist, echo = FALSE, results = 'asis', message = FALSE, warning = FALSE, include = FALSE}
# Copied no changes
cat("<details><summary><strong>Show figure: Posterior P-Value for logRS</strong></summary>\n")
...
```{r pval ggplot, echo = FALSE, results = 'asis', message = FALSE, warning = FALSE}
cat("<details><summary><strong>Show figure: Posterior P-Value for logRS</strong></summary>\n")

par(mfrow = c(1,1))

ggplot(as.data.frame(mean_simlogRS), aes(x = mean_simlogRS)) + 
...
```{r pvalpp, echo = FALSE, message = FALSE, warning = FALSE}
...
<!-- REMOVE COMMENT OUT WHEN FIXED: Where the posterior p-value represents the proportion of simulated means above the observed mean. In this case a value of `r pvalpp`, which indicates a slightly higher bias of the simulated responses over the observations. -->

### Residuals of Posterior Predictive against Years and Observed Spawners

There were visual trends in the temporal relationships of each population, but not in consistent direction. The “strong temporal trends” found by Liermann et al. (2010) were in reference to the base model. Given our intermediate version, in between the base-model and the time-varying alpha, it is expected to see temporal trends .

```{r residpp by year prep, echo = FALSE, results = 'asis', warning = FALSE, message = FALSE}
...
```{r pval per pop logRS, echo = FALSE, results = 'asis', warning = FALSE, message = FALSE}
# pvalpp <- sum(mean_simlogRS > mean(dat$logRS))/length(mean_simlogRS)
# Need a PER STOCK AND PER OBSERVATION simlogRS and obslogRS
poplogRS <- tempnumbersrdat |> 
  left_join(derived_obj$deripost_summary$logRS_pred, by = join_by(chain == Stock))

# Calculate P-VALUE PER STOCK
ppvallogRS <- tapply(sum(x > y)/length(x))

result <- poplogRS %>%
  group_by(Name) %>%
  summarise(
    summary_value = sum(Median > (log(Rec) - log(Sp))) / n()
  ) |> 
  left_join(WAbase, by = 'Name') |> 
  arrange(Latitude)
# No populations are above 0.9 or below 0.1
```
...
```{r pval resid logRS, echo = FALSE, results = 'asis', warning = FALSE, message = FALSE, include = FALSE}
# Posterior P-value for residauls of logSREP_re by watershed area and latitude
```
