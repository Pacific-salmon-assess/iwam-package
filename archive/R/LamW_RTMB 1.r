...
## This is what you would import. Only defined for a scalar. Need to update code if you want to be able to do more than 1 x at a time.
LambertW0 <- RTMB:::ADjoint(LambertW0_internal, dLambertW0_internal)

## Test it:
x <- seq(-10,10, by = 0.1)
F <- RTMB::MakeTape(function(x)sum(LambertW0(x)), numeric(1))

x <- x2 <-NULL
z <- seq(-1, 10, length = 50)
for( i in  z) x <- c(x, F(i))
for( i in  z) x2 <- c(x2, gsl::lambert_W0(i))

plot(z, x2, col = 'red')  ## Matches gsl.
lines(z, x, col = 'blue')

F$jacobian(-0.2)  ## This is working. I assume it is totally fine.