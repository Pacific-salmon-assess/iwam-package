...
# [16] "E_line_ocean"  "logAlphaSD"    "logESD"  
  ...
  # if (model == 'BETA'){
  # matrices$logBETA_tar_adj <- apply(matrices$logBETA_tar, 2, 
    # FUN = function(x)rnorm(length(x), x, sd = matrices$logBETA_sd))

  # matrices$logBETA_line_ocean_adj <- apply(matrices$logBETA_line_ocean, 2, 
    # FUN = function(x)rnorm(length(x), x, sd = matrices$logBETA_sd))
	
  # matrices$logBETA_line_stream_adj <- apply(matrices$logBETA_line_stream, 2, 
    # FUN = function(x)rnorm(length(x), x, sd = matrices$logBETA_sd))

  # matrices$logAlpha_tar_adj <- apply(matrices$logAlpha_tar, 2, 
    # FUN = function(x)rnorm(length(x), x, sd = matrices$logAlpha_sd))

  # matrices$BETA_tar_adj <- exp(matrices$logBETA_tar_adj) 
  # matrices$SMAX_adj <- 1 / matrices$BETA_tar_adj
  # matrices$SREP_adj <- matrices$logAlpha_tar_adj/matrices$BETA_tar_adj
  # matrices$SMSY_adj <- (1 - LambertW0(exp(1 - matrices$logAlpha_tar_adj))) / matrices$BETA_tar_adj
  # matrices$SGEN_adj <- -1/ matrices$BETA_adj * 
    # LambertW0(- matrices$BETA_tar_adj * matrices$SMSY_adj / (exp(matrices$logAlpha_tar_adj)))

  # matrices$BETA_line_ocean_adj <- exp(matrices$logBETA_line_ocean_adj) 
  # matrices$BETA_line_stream_adj <- exp(matrices$logBETA_line_stream_adj)
	# }
