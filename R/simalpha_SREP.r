...
simalpha <- function(bsiters = 1000, # Simulation iterations
	...
	poplen <- ncol(dsrep$deripost_full$Alpha_tar_adj) # 25 stock predictions to be made
	
	logSREP_sd <- dsrep$deripost_summary$logSREP_sd$Mean
	
	b0 <- dsrep$deripost_full$b0 # stream, +ocean
	bWA <- dsrep$deripost_full$bWA
	Alpha0 <- exp(dsrep$deripost_full$Alpha0)
	
	post <- cbind(Alpha0, b0[,1], bWA[,1]) # Full posteriors for all
	post2 <- cbind(exp(dsrep$deripost_full$Alpha0 + dsrep$deripost_full$Alpha02), b0[,1] + b0[,2], bWA[,1] + bWA[,2])
	...
	## From https://statproofbook.github.io/P/mvn-cond.html
		...
		newlogSREP <- ifelse(WAin$lh == 0, bnew[1] + bnew[2] * logWAshifted + rnorm(poplen, 0, sd = logSREP_sd),
											b2new[1] + b2new[2] * logWAshifted + rnorm(poplen, 0, sd = logSREP_sd))
		...
		SGENcalcs <- purrr::map2_dfr (exp(rep(logalphak, poplen)), exp(newlogSREP), Sgen.fn4) # @@@@
		
		# SREPold <- dsrep$deripost_summary$SREP$Median
