...
constants <- list(
  ...
  N_Obs = nrow(srdat),
  logWAbase = log(WAbase$WA) - 7.3, # mean(log(WAIn$WA)) - subtracting is same as division on log scale
  S = srdat$Sp, 
...
mean_logWA <- 7.3
pred.line <- data.frame(expand.grid(logWA = 4:16-mean_logWA, type = 1:2))
...
stock_recruit_srep_biasCor <- nimbleCode({
  ...
  # b0[1] ~ dflat() 
  b0[1] ~ dnorm(0, tau = 31)  ## Flat intercept for stream
  ...
  for(i in 1:N_Obs){ # 501 iterations - number of base observations
    ...
    logRS[i] ~ dnorm( logRS_pred[i], tau = tauobs[stk[i]])
  ...
  for( i in 1:npred.line){
    ...
    beta_pred_line[i] <- log_alpha_line[i]/E_pred_line[i] # Pred beta
  ...
  for( i in 1:npred){
    ...
    beta_pred[i] <- log_alpha[i]/E_pred[i] # Pred beta
...
```{r model stock_recruit_srep_biasCor_2, echo=FALSE}
stock_recruit_srep_biasCor_2 <- nimbleCode({
  ...
  b0[2] ~ dnorm(0, tau = 5)  ## Flat intercept for ocean
  bWA[1] ~ dnorm(0, tau = 1)  ## log Watershed area slope for stream
  bWA[2] ~ dnorm(0, tau = 5)  ## log Watershed area slope for ocean
  ...
  for( i in 1:N_Stk ) { # 25 iterations - number of base stocks
    ...
    log_E[i] <- b0[1] + b0[2]*typeOcean[i] + bWA[1]*logWAbase[i] + bWA[2]*logWAbase[i]*typeOcean[i] + logE0[i] ## Stock level regression
    # according to Liermann there should be something like:
      # E <- a + a*lifehist[i] + (b + b*lifehist[i])*logWAbase[i] + logE0[i]
    # log_E[i] <- b0 + b0_stream*type[i] + (bWA + bWA_stream*type[i])*logWAbase[i] + logE0[i]
    # where Here lifehist[i] is the life history type (Lj = 0 for stream-type and Lj = 1 for ocean-type).
  ...
  for ( j in 1:2 ){ # STREAM AND OCEAN TYPE
    ...
    ## Pred Srep
    # log(E_pred[i]) <- b0 + bWA*logWAPred[i] # + logE_re[i]
    log(E_pred[i,j]) <- b0[1] + b0[2]*(j-1) + (bWA[1] + bWA[2]*(j-1))*logWAPred[i] # + logE_re[i]
    # log(E_pred[i,j]) <- b0[j] + bWA[j]*logWAbackPred[i]
      # Stick without logE_re for now
      # remove logE_re - assumes that there is no random effect for a site-specific effect for the prediction - "conditional mean"
        # confidence - how uncertain you are about expected value
...
```{r model run, echo=FALSE, warning=FALSE, message=FALSE}

# Model 2:
# constants$typeOcean <- (constants$type == 2)*1
# RmodelSrep <- nimbleModel(stock_recruit_srep_biasCor_2, data = data, constants = constants, 
...
# Model 2:
# conf2 <- configureMCMC(RmodelSrep, control = list(adaptFactorExponent = 0.1))
# conf2$removeSamplers(c("b0", "bWA"))
# conf2$addSampler(target =c("b0[1]"), type = 'slice')
# conf2$addSampler(target =c("bWA[1]"), type = 'slice')
# conf2$addSampler(target =c("b0[2]"), type = 'slice')
# conf2$addSampler(target =c("bWA[2]"), type = 'slice')

conf2$setMonitors(c('logAlpha', 'logAlpha0', 'E', 'log_E', "b0", "bWA", "tauobs",
                    "logAlphaSD", "logESD", "logE0", 
                    "E_pred", "beta_pred", "log_alpha","predSmsy",
                    "log_alpha_line", "beta_pred_line", "predSmsy_line", "E_pred_line"))
...
MCMCtrace(object = mcmc.out,
          ...
          params = "tauobs")
# E_pred - SMSY predicted
# b0 - intercept estimates
# bWA - slope estimate
# logAlpha0
# logAlphaSD
```
...
```{r sgen calc, echo=FALSE, message=FALSE, warning=FALSE}
# sgen calculation
# log_alpha = alpha
# E_pred = SREP
