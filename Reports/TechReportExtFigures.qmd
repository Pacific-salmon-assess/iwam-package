...
# fitstan_prioronly <- load(here::here("R/fitstan_prioronly.RData"))
# derived_obj_prioronly <- get(load(here::here("R/derived_obj_prioronly.RData")))
```
...
f_srep <- function(par){
  ...
  for (i in 1:N_Pred){
    ...
    logSREP_tar[i] <- b0[1] + b0[2]*type_tar[i] + (bWA[1] + bWA[2]*type_tar[i])*WAin$logWAshifted_t[i] + biaslogSREP logSREP_sd^2/2
...
# heidel_results[[1]][,] # produces the raw output of heidel.diag() from coda
```
...
```{r residpost plot1 RAW, echo = FALSE, results = 'asis', warning = FALSE, message = FALSE}
# cat("<details><summary><strong>Show figure: Residuals (logSREP_re x logSREP_sd)</strong></summary>\n")
...
```{r residpost plot2 RAW, echo = FALSE, results = 'asis', warning = FALSE, message = FALSE}
# cat("<details><summary><strong>Show figure: Residuals (logSREP_re)</strong></summary>\n")

# ggplot(data = dflogSREP_re, aes(x = logWAshifted, y = value)) + 
# 	geom_point(alpha = 0.5) + 
#   theme_classic() + 
# 	labs(y = "Standardized Residual of WA Relationship", x = "Log Centered WA")
...
res1oc <- ggplot(data = dflogSREP_re[dflogSREP_re$lh == 'ocean',], aes(x = logWAshifted, y = value)) + 
...
res2oc <- ggplot(data = dflogSREP_re[dflogSREP_re$lh == 'ocean',], aes(x = Latitude, y = value)) + 
	geom_point(alpha = 0.1, color = 'skyblue') + 
...
res1st <- ggplot(data = dflogSREP_re[dflogSREP_re$lh == 'stream',], aes(x = logWAshifted, y = value)) + 
...
res2st <- ggplot(data = dflogSREP_re[dflogSREP_re$lh == 'stream',], aes(x = Latitude, y = value)) + 
	geom_point(alpha = 0.1, color = 'forestgreen') + 
...
# # qqline(residplot$value, col = 'red')
# abline(0,1)
...
```{r resid logSREP_re acf plot, echo = FALSE, results = 'asis', warning = FALSE, message = FALSE}
# ACF plot for residual: logSREP_re

# resid_mcmc_list <- mcmc.list(lapply(chains_list, as.mcmc))
# autocorr.plot(resid_mcmc)
# mcmc_acf(resid_draws, lags = 50)

# drawslogSREP_re <- as_draws_matrix(derived_obj$deripost_full$logSREP_re)

# cat("<details><summary><strong>Show figure: Residuals ACF of Watershed Area Model</strong></summary>\n")
# bayesplot::mcmc_acf(drawslogSREP_re)
# cat("</details>\n\n")
```

```{r resid logRS acf prep, echo = FALSE, results = 'asis', warning = FALSE, message = FALSE}
# ACF plot for residual: logRS(observed) - logRS_pred
# Autocorrelation - one plot for each stock --> Autocorrelation of that timeseries
# Needs to be divided up into folds depending on what stock it is related to?

# Data Extraction:
# drawslogRS <- as_draws_matrix(residlogRS) OR could use 'residplot'
```

```{r resid logRS acf plot, echo = FALSE, results = 'asis', warning = FALSE, message = FALSE}
# a 5x5 grid of PER stock plots
# PER stock - will be one ACF plot for the XXX number of observations for that stock - out of the 501 total
...
# for (i in 1:25) {
# 	acf(residplot_median$median_value[residplot_median$Stocknumber == i - 1])  
# }
