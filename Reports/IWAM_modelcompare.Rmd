...
1.  Evaluation <!--CH Estimation instead of Evaluation--> of the Ricker model using a Chinook spawner and recruit time-series from across the NE Pacific <!--CH add the number of time-series currently used-->
...
$S$ is  spawner abundance expressed as a $numeric$. Is it standard that both recruits and spawners be scaled values for modelling. 
...
Where $\tau \sim Normal(-\sigma_\tau^2/2, \sigma_\tau)$, <!--CH : see line 123 of IWAM_Liermann.cpp-->

and the mean is adjusted by -\sigma_\tau^2/2 to account for the log back-transformation adjustment.
...
and $\sigma_\alpha_i \sim InvGamma(0.01,0.01)$.
...
### The Liermann et al. 2010 Model Type
...
and, $S_{REP}$ is the point on the curve where $R = S$, also defined as the unfished equilibrium point.
...
<!--CH: it lools like there are two vesrions of these equations here: and below for linearized form, but the priors ar diferent. Which one was used? Remove the one that is not used.-->
...
<!--CH: I think log\alpha should be alpha, as defined above, and suggest changing E to SREP, as i mentioned above. Also, add $w_{ij}$:-->
