...
-   Walkthrough the outputs of both models
...
3.  Using the regression model estimate *new* Ricker model parameters for stocks where only watershed area is known
...
-   Optimize for an estimate of $S_{GEN}$ using the supplied $\alpha$ and new $\beta$, where
...
-   Next steps in model development
    ...
    -   Consideration and discussion around $S_{REP}$ and $S_{MAX}$ as management parameters. This would include a potential re-parameterization of the model for $S_{MAX}$
    -   Translations of both models into RTMB
    -   Incorporation of a regional varying $\alpha$ parameter
    -   Implementation of the Liermann random-walk $\alpha$
...
The TMB model for the Holt implementation can be accessed through a single function: `IWAM_func` from the IWAM repository of Pacific-Salmon-Assess. The function requires only the input of stocks and watersheds for those that you wish to estimate sotck recruitment parameters for. In the case below: `WCVIStocks.csv`. The remaining function defaults are to control the presence or absense of enhancement, and the parameters of the bootstrapping.
...
```{r mcmc traces, echo=FALSE, include=FALSE}
...
```{r nimble residuals, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
#### Stock-wise comparisons of parameters {.tabset}
...
SREP_f <- SREP_p +
  geom_point(WAin, mapping = aes(x = Stock, y = Srep_Mean), color='blue') +
  geom_errorbar(data = WAin, aes(x = Stock, ymax = `Srep_97.5%`, ymin = `Srep_2.5%`), color='blue', width = 0.2, inherit.aes = FALSE)
...
SMSY_f <- SMSY_p +
  geom_point(WAin, mapping = aes(x = Stock, y = Smsy_Mean), color='blue') +
  geom_errorbar(data = WAin, aes(x = Stock, ymax = `Smsy_97.5%`, ymin = `Smsy_2.5%`), color='blue', width = 0.2, inherit.aes = FALSE)
...
```{r tmb SREP wa reg, include=FALSE, eval=FALSE}
# IWAM_WCVI_noEnh --> 
...
```{r tmb SMSY wa reg 2, include=FALSE}
```

##### NIMBLE

```{r nimble wa reg, include=FALSE}
ggplot(data = WAline, aes(x = logWA, y = log(Sgen_Mean), colour = lh, fill = lh)) +
  geom_line() + 
  geom_ribbon(aes(ymin = log(`Sgen_2.5%`), ymax = log(`Sgen_97.5%`)), alpha = .1) +
...
```{r nimble old plots, echo=FALSE, include=FALSE}
