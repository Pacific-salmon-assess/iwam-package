...
1.  Summary
2.  Data Map: Code repositories, where to find data
3.  IWAM Model Package Code
4.  Final Estimate Bootstrapping
5.  SREP, SGEN, SMSY Estimates

## S1. Document Summary

Lorem ... - Reference to comparison paper:
<https://publications.gc.ca/collections/collection_2023/mpo-dfo/fs70-5/Fs70-5-2023-010-eng.pdf>

## S2. Data Map

The following worked example is from the IWAM Package Repo hosted
publicly through the Pacific Salmon Assess organization available at
[this link.](https://github.com/Pacific-salmon-assess/iwam-package)
...
-   *CUPars_nBC (Public)*: Input file with Ricker $\alpha$'s from Diana Dobson's Run Reconstruction coded in TMB. Has a higher estimate of Ricker $\alpha$ (lower Sgen).
-   *CUPars_wBC (Public)*: Input file with Ricker $\alpha$'s from Diana Dobson's Run Reconstruction coded in TMB. Has a higher estimate of Ricker $\alpha$ (lower Sgen).
...
## S3: IWAM Model (Produce initial SREP estimates)
...
Summary of data setup ...

``` {r iwam data, include=FALSE}
...
stream <- srdat %>% dplyr::select(Stocknumber, Name, lh) %>% #srdat Stream is still capcased
  group_by(Stocknumber) %>% 
  summarize(lh=max(lh)) %>% 
  arrange (Stocknumber)
```

```{r OLD iwam data, eval=FALSE, include=FALSE, echo=FALSE}
# IWAM Model Setup
remove.EnhStocks <- TRUE
...
``` {r data and params, include=FALSE}
...
```{r OLD data list, eval=FALSE, message=FALSE, include=FALSE}
data <- list()
...
```{r OLD stock data, eval=FALSE, include=FALSE}
data$target_lnWA_ocean <- wcvistocks %>% # PUBLIC
  mutate (lnWA=log(WA)) %>%
  filter(lh==1) %>% 
  pull(lnWA)
...
```{r OLD params list, eval=FALSE, include=FALSE}
param <- list()
...
Summary of IWAM model running/setup ...

What data is output ...

``` {r iwam model run, include=FALSE}
...
```{r OLD iwam model, eval=FALSE, include=FALSE, echo=FALSE}
# IWAM Model Run
mod <- "IWAM_Liermann" 
# dyn.unload(dynlib(paste("TMB_Files/", mod, sep="")))
# compile(paste("TMB_Files/", mod, ".cpp", sep=""))
...
```{r iwam plotting 1, include=TRUE, echo=FALSE}
...
```{r iwam plotting 2, include=FALSE, echo=FALSE}
PlotSRLinear(srdat=srdat, pars=pars, r2=r2, removeSkagit = FALSE)
# *Figure 2. Stock-Recruitment Linear Relationship per WCVI Stock. INCLUDE
# UNITS.*
```

```{r iwam plotting 3, include=FALSE, echo=FALSE}
PlotStdResid(SRes)
# *Figure 3. Standard Residuals. INCLUDE UNITS.*
```

```{r iwam plotting 4, include=FALSE, echo=FALSE}
Plotacf(SRes)
# *Figure 4. ACF. INCLUDE UNITS.*
```

``` {r wa regressions, echo=FALSE, message=FALSE, fig.show="hold", out.width="50%"}
# require(gridExtra)
# library(cowplot)
# par(mfrow=c(1,1), mar=c(4, 4, 4, 2) + 0.1)
title_plot_SMSY <- "Prior Ricker sigma and prior WA regression sigma"
title_plot_SREP <- "Prior Ricker sigmas and prior on WA regression sigma"

plot_SMSY <- plotWAregressionSMSY (pars, all_Deltas, srdat, stream, WA, pred_lnSMSY, 
                        pred_lnWA = data$pred_lnWA, title1=title_plot_SMSY, mod)
plot_SREP <- plotWAregressionSREP (pars, all_Deltas, srdat, stream, WA, pred_lnSREP, 
                        pred_lnWA = data$pred_lnWA, title1=title_plot_SREP, mod)

# plot_grid(plot_SMSY, plot_SREP, labels = "AUTO")
# grid.arrange(plot_SMSY, plot_SREP, nrow=2)

```
*Figure 6. (Left) Prior Ricker sigmas and prior on WA regression for log(SMSY) and log(Watershed Area (km\^2)).(Right) Prior Ricker sigmas and prior on WA regression for log(SREP) and log(Watershed Area (km\^2)).*

### Output Data from IWAM Model

What data is put out ...

-   predictions lnsmsy and lnsrep (pi)
...
# **** Include here::here syntax for stream lines below ****
...
```{r OLD iwam data out, eval=FALSE, include=FALSE}
pred_lnSMSY_pi <- data.frame()
pred_lnSMSY_pi <- all_pars %>% filter   (Param %in% c("pred_lnSMSY", "lnSMSY"))
pred_lnSREP_pi <- data.frame()
pred_lnSREP_pi <- all_pars %>% filter (Param %in% c("pred_lnSREP", "lnSREP"))
...
## S3: Bootstrapping and final estimates of SREP, SGEN, and SMSY

The following section includes the code for bootstrapping and the re-calculation of the SREP, SGEN, and SMSY estimates for the West-Coast Vancouver Island Stocks (Table 1-3). The following stocks of; Barkley, Clayoquot, Kyuquot, Nootka/Esperanza, and Quatsino, are displayed for direct comparison, however all stocks are available and can be accessed in this example if further comparison is desired. The bootstrap was run in this case with 10 trails to reduce run time and for ease of sharing. To change the number of trails - please see the `{r} nBS` object. The final estimates include the median (50%) their 2.5% and 97.5% quantiles, rounded to two significant figures.
...
if (run.bootstraps){
  ...
  for (k in 1:nBS) {
    out <- Get.LRP.bs(datain = "DataOut/WCVI_target_ocean_noEnh.csv", run_logReg=FALSE) 
  ...
  stockNames <- read.csv(here::here("DataOut/WCVI_target_ocean_noEnh.csv")) %>% # datain as called by Get.LRP.bs()
