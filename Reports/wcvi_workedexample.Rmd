...
The following worked example is from the IWAM Package Repo hosted publicly through the Pacific Salmon Assess organization available at [this link.](https://github.com/Pacific-salmon-assess/iwam-package)
...
# Original code for Inlet and CU aggregations for reference
...
# Original separation code for reference:
# if(remove.EnhStocks) data$target_lnWA_ocean <- c(data$target_lnWA_ocean, 
...
if (length(stocknames_ocean) == length(length_check_params_ocean$Estimate)) {
...
# Old Loop of Stream for reference:
...
run.bootstraps <- TRUE
# make sure datain is saved - see above sections
...
if (run.bootstraps){
  ...
  for (k in 1:nBS) {
    out <- Get.LRP.bs(datain = datain,                                                         Bern_logistic=FALSE,
                      prod="LifeStageModel",
                      LOO=NA,
                      run_logReg=FALSE,
                      ) 
  ...
  stockNames <- read.csv(here::here(datain)) %>% # datain as called by Get.LRP.bs()
    # filter(Stock != "Cypre") %>% 
    pull(Stock)
  ...
  write.csv(dfout, here::here("DataOut/workedexample-BootstrappedRPs.csv"))
