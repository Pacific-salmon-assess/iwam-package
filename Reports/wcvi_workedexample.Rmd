...
WAbase <- read.csv(here::here("DataIn/WatershedArea.csv")) # PRIVATE
# Add WCVIstocks # PUBLIC
WAin <- read.csv(here::here("DataIn/WCVIStocks.csv")) # old wcvistocks
...
lifehist <- srdat %>% dplyr::select(Stocknumber, Name, Stream) %>% #srdat Stream is still capcased
...
plot_SMSY <- plotWAregressionSMSY (pars, all_Deltas, srdat, lifehist, WAbase, pred_lnSMSY, 
                        pred_lnWA = data$pred_lnWA, title1=title_plot_SMSY, mod)
plot_SREP <- plotWAregressionSREP (pars, all_Deltas, srdat, lifehist, WAbase, pred_lnSREP, 
