...
For both the $S_{MAX}$ and $S_{REP}$ models the solution to $S_{MSY}$ is solved with a Lambert W equation defined by Schueurel (2016). The only exceptions are that we use an explicit form of LambertW, where the original SMAX model form had a non-explicit optimization method.
...
The alternative version requires an optimisation of 
